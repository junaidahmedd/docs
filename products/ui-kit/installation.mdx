...
Revo UI Kit installs like a normal FiveM resource, but it also needs a required loader snippet added to `ox_lib`. If you also want the redesign to apply to `ox_target`, you must replace the `ox_target` web `index.html` file as well.
...
This loader checks whether `revo_uikit` exists and, if it does, loads its `init.lua` through `ox_lib`.

That allows Revo UI Kit to hook into `ox_lib` and replace supported UI parts safely.

If the resource is missing, the loader does nothing.

If loading fails, it throws a clear error so you can troubleshoot the integration.
...
### What This ox_target File Does

This version of `index.html` checks whether Revo UI Kit's web bundle is available.

Behavior:

- if `revo_uikit` is running, `ox_target` loads the Revo UI Kit interface
- if `revo_uikit` is missing or unavailable, `ox_target` falls back to its normal default UI

This makes the `ox_target` integration safe and prevents it from fully breaking if Revo UI Kit is not active.
...
5. If `revo_uikit` is stopped, the fallback behavior works correctly for `ox_target`

<Tip>
  If the UI does not appear correctly, check the `ox_lib/init.lua` integration first. That is the required step for the script to hook into the supported UI systems.
</Tip>
